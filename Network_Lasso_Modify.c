...
#define MAX_RUN 1
#define MAX_ITER 1
#define TRAIN_RHO 0.09 //step size for solving training problem
#define TEST_RHO 0.09 //step size for solving testing problem
...
printf("\n");
    ...
    //decision variables for edges:
    double ***test_z = (double ***)malloc(sizeof(double **)*NUM_TEST);
    for (i = 0; i < NUM_TRAIN; i++){
        test_z[i] = (double **)malloc(sizeof(double *)*4);
        for (j = 0; j < 4; j++){
            test_z[i][j] = (double *)malloc(sizeof(double)*test_neighbor_count[i]);
        }
    }
    //Open dual variables for testing data:
    //correctors:
    double ***test_lambda = (double ***)malloc(sizeof(double **)*NUM_TEST);
    for (i = 0; i < NUM_TEST; i++){
        test_lambda[i] = (double **)malloc(sizeof(double *)*4);
        for (j = 0; j < 4; j++){
            test_lambda[i][j] = (double *)malloc(sizeof(double)*test_neighbor_count[i]);
        }
    }
    //predictors:
    double ***test_p = (double ***)malloc(sizeof(double **)*NUM_TEST);
    for (i = 0; i < NUM_TEST; i++){
        test_p[i] = (double **)malloc(sizeof(double *)*4);
        for (j = 0; j < 4; j++){
            test_p[i][j] = (double *)malloc(sizeof(double)*test_neighbor_count[i]);
        }
    }
    ...
    double test_edge_objval_singular, test_edge_objval_non_singular;
...
printf("%d th iteration:\t", iter);
            ...
            for (i = 0; i < train_num_edge; i++){
                //update decision variables for edges:
                for (j = 0; j < 4; j++){
                    a[j] = (1/TRAIN_RHO)*train_p[train_edge[i][0]][j][train_edge[i][2]] + train_z[train_edge[i][0]][j][train_edge[i][2]];
                    b[j] = (1/TRAIN_RHO)*train_p[train_edge[i][1]][j][train_edge[i][3]] + train_z[train_edge[i][1]][j][train_edge[i][3]];
                }
                c = w*train_weight[i];
                theta = 0.0;
                for (j = 0; j < 4; j++){
                    theta += pow((double)(a[j] - b[j]), (double)2);
                }
                theta = sqrt(theta);
                theta = 1 - c/(TRAIN_RHO*theta);
                if (theta < 0.5){
                    theta = 0.5;
                }
                for (j = 0; j < 4; j++){
                    train_z[train_edge[i][0]][j][train_edge[i][2]] = theta*a[j] + (1 - theta)*b[j];
                    train_z[train_edge[i][1]][j][train_edge[i][3]] = (1 - theta)*a[j] + theta*b[j];
                }
                difference = 0.0;
                for (j = 0; j < 4; j++){
                    difference += pow((double)(train_z[train_edge[i][0]][j][train_edge[i][2]] - train_z[train_edge[i][1]][j][train_edge[i][3]]), (double)2);
                }
                train_objval += c*sqrt(difference);
            }
...
printf("%lf\t%lf\n", train_objval, train_residual);
        ...
        //Initiate primal variables for testing data:
        //decision variables for nodes:
        for (i = 0; i < NUM_TEST; i++){
            for (j = 0; j < 4; j++){
                test_x[i][j] = 0.0; //initial value
            }
        }
        //decision variables for edges:
        for (i = 0; i < NUM_TEST; i++){
            for (j = 0; j < 4; j++){
                for (k = 0; k < test_neighbor_count[i]; k++){
                    test_z[i][j][k] = 0.0; //initial value
                }
            }
        }
        //Initiate dual variables for testing data:
        //correctors:
        for (i = 0; i < NUM_TEST; i++){
            for (j = 0; j < 4; j++){
                for (k = 0; k < test_neighbor_count[i]; k++){
                    test_lambda[i][j][k] = 0.0;
                }
            }
        }
...
printf("%d th iteration:\t", iter);
            ...
            for (i = 0; i < NUM_TEST; i++){
                for (j = 0; j < 4; j++){
                    for (k = 0; k < test_neighbor_count[i]; k++){
                        test_p[i][j][k] = test_lambda[i][j][k] + TEST_RHO*(test_x[i][j] - test_z[i][j][k]);
                    }
                }
            }
            ...
            for (i = 0; i < NUM_TEST; i++){
                for (j = 0; j < 4; j++){
                    for (k = 0; k < test_neighbor_count[i]; k++){
                        test_x[i][j] += -(1/TEST_RHO)*test_p[i][j][k];
                    }
                }
            }
            ...
            for (i = 0; i < NUM_TEST; i++){
                for (k = 0; k < test_neighbor_count[i]; k++){
                    ...
                    //singular point:
                    test_edge_objval_singular = 0.0;
                    for (j = 0; j < 4; j++){
                        test_edge_objval_singular += test_p[i][j][k]*(test_x[i][j] - train_x[test_neighbor_index[i][k]][j]) + 0.5*TEST_RHO*pow((double)(train_x[test_neighbor_index[i][k]][j] - test_z[i][j][k]), (double)2);
                    }

                    //compare objective function values at non-singular point with singular point:
                    if (test_edge_objval_non_singular < test_edge_objval_singular){
                        for (j = 0; j < 4; j++){
                            test_z[i][j][k] = ;
                        }
                    }
                        ...
                        for (j = 0; j < 4; j++){
                            test_z[i][j][k] = train_x[test_neighbor_index[i][k]][j];
                        }
                        test_objval += 0.0;
                    }
                }
            ...
            for (i = 0; i < NUM_TEST; i++){
                for (j = 0; j < 4; j++){
                    for (k = 0; k < test_neighbor_count[i]; k++){
                        test_lambda[i][j][k] += TEST_RHO*(test_x[i][j] - test_z[i][j][k]);
                        test_residual += pow((double)(test_x[i][j] - test_z[i][j][k]), (double)2);
                    }
                }
            }
            test_residual = sqrt(test_residual/test_neighbor_total);
printf("%lf\t%lf\n", test_objval, test_residual);
            if (test_residual < TAULERANCE){
                break;
            }
        }
